...
def save_to_mysql(recurso_id, timestamp, pue, carbon, cpu, power, energia_renovable):
    ...
    try:
        ...
        query = """INSERT INTO consumo (id_recurso, timestamp, pue, carbon, cpu_usage, power_cons, renovable)
                   VALUES (%s, %s, %s, %s, %s, %s, %s)"""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
         
        cursor.execute(query, (recurso_id, timestamp, pue, carbon, cpu, power, energia_renovable))
        conn.commit()
        cursor.close()
        conn.close()
    except Exception as e:
...
def metrics():
   ...
   # Calculo de la huella de carbono (0.4 kg CO2/kWh por defecto)
    ...
    save_to_mysql(recurso_id, timestamp, pue, carbon, cpu, power, energia_renovable)
